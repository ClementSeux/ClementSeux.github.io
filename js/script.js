...
$(function ($) {
  ...
  function getStarCoordinate() {
    ...
    const x = getRandomInt(
      Math.floor(canvas.width * margin),
      Math.floor(canvas.width * (1 - margin))
    );
    const y = getRandomInt(
      Math.floor(canvas.height * margin),
      Math.floor(canvas.height * (1 - margin))
    );
    const starSize =
      Math.floor(window.devicePixelRatio * Math.random() * 2) + 1;
    return [x, y, starSize];
  }
  ...
  if (canvas.getContext) {
    ...
    function speedStars(context) {
      ...
      for (let i = 0; i < starAtlas.length; i++) {
        ...
        context.lineTo(
          starAtlas[i][0] + ratio * (starAtlas[i][0] - center[0]),
          starAtlas[i][1] + ratio * (starAtlas[i][1] - center[1])
        );
